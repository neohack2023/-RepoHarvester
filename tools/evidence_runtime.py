#!/usr/bin/env python3
"""Deterministic evidence relations for RepoHarvester DevOS.

This module implements the first executable Learning Spine slice:

atomic finding -> current-state cross-reference -> lineage/root collapse
-> independent-root triangulation -> explicit delta packet

It intentionally does not perform semantic inference, authority promotion, reflection,
curriculum selection, or live external-memory retrieval.
"""
...
def cross_reference(
    ...
    """Classify one finding against explicit current-state claim identity.

    Semantic claim identity is deliberately supplied by ``claim_key`` rather than
    inferred here. This keeps the executable stage deterministic and inspectable.
    """
    ...
    if current is None:
        if finding.related_claim_keys:
            return FindingAssessment(finding, CrossReference.PARTIAL_OVERLAP, None)
        return FindingAssessment(finding, CrossReference.NEW, None)
...
def summarize_roots(findings: Sequence[AtomicFinding]) -> RootSummary:
    """Collapse derivative artifacts while preserving correlation/unknown states."""
    ...
    for finding in findings:
        ...
        if state in COUNTABLE_INDEPENDENT_STATES:
            ...
            # Derivatives retain their root references but never add an
            # independence vote. If the root is independently represented by
            # another finding it will already be counted there.
            continue

    # A root explicitly marked correlated must not also count as maximally
...
def triangulate(
    ...
    *,
    ...
    """Triangulate findings for exactly one atomic claim key."""
    ...
    findings = [a.finding for a in scoped]
    ...
    elif summary.correlated_root_count or summary.reproduction_count:
        state = TriangulationState.INSUFFICIENT
    else:
        state = TriangulationState.INSUFFICIENT
...
def build_delta_packet(
    ...
    """Build a deterministic, authority-neutral delta packet."""
    ...
    material_states = {
        CrossReference.NEW.value,
        CrossReference.CONTRADICTS.value,
        CrossReference.SUPERSEDES.value,
    }
    material = bool(material_states.intersection(states)) and triangulation.state not in {
        TriangulationState.INSUFFICIENT,
        TriangulationState.ORTHOGONAL,
    }
    ...
    delta_id = "delta-" + hashlib.sha256(canonical.encode("utf-8")).hexdigest()[:16]

    return DeltaPacket(delta_id=delta_id, **payload)
